...
<template>
  <main>
    ...
    <Form @submit="onSubmit" :validation-schema="registerSchema" v-slot="{ meta }" class="form">
      ...
      <section class="form__section">
        ...
        <div class="form__section__1-1">
          <TextField v-model="form.altura" :meta="meta" name="altura" type="number" label="Altura(cm)" required="*"
            placeholder="Ex: 180" />
          <TextField v-model="form.peso" :meta="meta" name="peso" type="number" label="Peso(kg)" required="*"
      ...
      <section class="form__section">
        ...
        <TextField v-model="form.q3" name="q3" :meta="meta" type="text"
          label="Em quanto tempo espera atingir esses objetivos?" />
        <TextField v-model="form.q4.treinando" name="q4a" :meta="meta" type="radio" :radios="q4Radio"
          label="Você está parado(a) ou treinando?" />
        <div class="q4time" :class="{ 'input--disabled': form.q4.treinando === undefined }">
          <TextField v-model="q4Calc" @input="pushToTime" name="q4b" :meta="meta" type="number" label="Há quanto tempo?"
        ...
        <TextField v-model="form.q5" name="q5" :meta="meta" type="textarea" rows="2"
          label="Como é ou era o seu último treino?" />
        <TextField v-model="form.q6" name="q6" :meta="meta" type="text"
          label="Existe algum exercício que você não gosta de fazer? Por que?" />
        <TextField v-model="form.q7" name="q7" :meta="meta" type="text"
          label="E qual exercício você ama fazer? Por que?" />
        <TextField v-model="form.q8" name="q8" :meta="meta" type="text"
          label="Em quanto tempo você concluía seu treino?" />
        <TextField v-model="form.q9" name="q9" :meta="meta" type="text" label="Tem restrição de tempo para treinar?"
          span="Um treino de 30 a 50 minutos seria suficiente!" />
        <TextField v-model="form.q10" name="q10" :meta="meta" type="text"
          label="Quanto tempo de treino você tem disponível por dia?" />
        <TextField v-model="form.q11" name="q11" :meta="meta" type="text" label="Qual será o local de treino?"
          span="(Academia; academia do prédio; casa; parque; etc)" />
        <TextField v-model="form.q12" name="q12" :meta="meta" type="number"
        ...
        <table :class="{ 'input--disabled': !form.q12 }">
          ...
          <tbody>
            <tr v-for="day in days" :key="day">
              <td>{{ day }}</td>
              <td>
                <input :name="`${day}`" :value="{ day, value: 1 }" type="checkbox"
                  @change="pushToTable({ day, value: 1 })" :disabled="disableCheckbox(day)"
                  :tabindex="!form.q12 ? '-1' : null" />
              </td>
              <td>
                <input :name="`${day}`" :value="{ day, value: 2 }" type="checkbox"
                  @change="pushToTable({ day, value: 2 })" :disabled="disableCheckbox(day)"
                  :tabindex="!form.q12 ? '-1' : null" />
              </td>
              <td>
                <input :name="`${day}`" :value="{ day, value: 3 }" type="checkbox"
                  @change="pushToTable({ day, value: 3 })" :disabled="disableCheckbox(day)"
                  :tabindex="!form.q12 ? '-1' : null" />
              </td>
            </tr>
          </tbody>
        ...
        <TextField v-model="form.q15" name="q15" :meta="meta" type="radio" :radios="YesOrNoRadio"
          label="Está sendo acompanhado(a) por nutrologista ou endocrinologista?" />
        <TextField v-model="form.q16" name="q16" :meta="meta" type="text"
          label="Descreva rapidamente sua rotina alimentar!" />
        <TextField v-model="form.q17" name="q17" :meta="meta" type="number"
          label="De 1 a 10 como você classifica seu sono." span="(Sendo 1 para péssimo e 10 para excelente)" />
        <TextField v-model="form.q18" name="q18" :meta="meta" type="text" label="Qual sua profissão?" />
        <TextField v-model="form.q19" name="q19" :meta="meta" type="text"
          label="Em sua profissão você fica muito tempo sentado(a), em movimento ou realiza trabalho braçal?" />
        <TextField v-model="form.q20" name="q20" :meta="meta" type="radio" :radios="YesOrNoRadio"
          label="Você tem algum tipo de dor ou desconforto (muscular ou articular)?" />
        <TextField v-model="form.q21" name="q21" :meta="meta" type="textarea" rows="2" v-show="form.q20"
          label="Se sim, onde? Leve ou aguda? Esporádica ou crônica? Qual a intensidade dessa(s) dor(es) de 0 a 10?" />
        <TextField v-model="form.q22" name="q22" :meta="meta" type="text" rows="2"
        ...
        <TextField v-model="form.q25" name="q25" :meta="meta" type="text"
          label="Seu médico sabe que você está ingressando em um programa de treinamento físico?" />
        <TextField v-model="form.q26" name="q26" :meta="meta" type="text" label="Você tem alguma meta para atingir?"
          span="Ex: uma data; uma festa ou evento; uma roupa; etc." />
        <TextField v-model="form.q27" name="q27" :meta="meta" type="textarea"
          label="Existe algo que você acredita ser relevante me contar para personalizar ainda mais o seu treino? Essa é a hora!!!" />
        <p class="final"> Vamos que vamos, pois juntos e com compromisso somos mais fortes! Obrigado pela confiança.</p>
      </section>
